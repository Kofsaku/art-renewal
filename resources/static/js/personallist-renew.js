...
function displayCurrentPage() {
    ...
    pageData.forEach(person => {
        ...
        columnOrder.forEach(columnKey => {
            ...
            switch (columnKey) {
                ...
                case 'gatePermissions':
                    ...
                    break;
                case 'actions':
                    cellContent = `<span class="text-muted small">ダブルクリック：編集 / Ctrl+クリック：履歴</span>`;
                    break;
                default:
        ...
        row.addEventListener('dblclick', () => {
            console.log(`🔥🔥 statusMonitor完全模倣ダブルクリック: ${person.name}`);
            showPersonEditModal(person.id, person.name);
        });

        row.addEventListener('contextmenu', (e) => {
            e.preventDefault();
            console.log(`🔥 statusMonitor完全模倣右クリック: ${person.name}`);
            showPersonHistoryModal(person.id, person.name);
        });

        // statusMonitorと同じ：appendChild
        tbody.appendChild(row);

        console.log(`✅ statusMonitor完全模倣: ${person.name}行作成完了`);
    });

    console.log('🔥 statusMonitor完全方式でテーブル構築完了:', tbody.children.length, '行生成');
    console.log('🎯 各行にイベントが個別設定済み（重複設定なし）');
}
